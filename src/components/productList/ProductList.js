...
const ProductList = (props) => {
  ...
  useEffect(() => {
    const fetchProductList = async () => {
      fetch(`https://api.chec.io/v1/products?category_slug=${categorySlug}`, {
        headers: getHeaders(),
      })
      ...
      <div className="container" style={footer}>
        <div className="row gap-3 pt-3">
          {filteredProducts.map((product) => (
            <div
              className="col d-flex flex-wrap flex-column border border-subtle border-2 text-center justify-content-center"
              key={product.id}
            >
              <Link
